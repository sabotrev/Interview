...
Description:
    Create a function taking a positive integer as its parameter and returning a string containing
        ...
        rendered: 1000=M, 900=CM, 90=XC; resulting in MCMXC. 2008 is written as 2000=MM, 8=VIII; or
...
Using map
*/
const romanNumeralEncoder1 = (num) => {
    const rm = {
        "M" : 1000,
        "CM": 900,
        "D" : 500,
    ...
    Object.keys(rm).map(val => {
        let n = Math.floor(num / rm[val]);
        num -= n * rm[val];
        result += val.repeat(n);
    });
...
console.log(romanNumeralEncoder1(1001)); // MI
console.log(romanNumeralEncoder1(60)); // LX
console.log(romanNumeralEncoder1(14)); // XIV